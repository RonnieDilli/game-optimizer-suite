...
class QueTelaApp(ctk.CTk):
    ...
    # ==================== COMPONENTE VISUAL UNIFICADO ====================
    def render_optimization_card(self, parent, item, apply_callback, is_active=False, modo="performance"):
        """
        Versão densa e alinhada com destaque didático.
        """
        is_bad = not is_active and modo == "performance"
        card_color = "#34495E" if is_active else ("#4A1919" if is_bad else "#2C3E50")

        card = ctk.CTkFrame(parent, fg_color=card_color, border_width=1, border_color="#555")
        card.pack(fill="x", pady=2, padx=5)

        # Container principal grid para alinhamento rígido
        container = ctk.CTkFrame(card, fg_color="transparent")
        container.pack(fill="x", padx=10, pady=5)
        container.grid_columnconfigure(0, weight=1) # Esquerda (texto)
        container.grid_columnconfigure(1, weight=0) # Direita (badges + btn)

        # --- COLUNA ESQUERDA ---
        left_col = ctk.CTkFrame(container, fg_color="transparent")
        left_col.grid(row=0, column=0, sticky="ew")
        title_text = f"{item.get('nome')} {'⚠️' if is_bad else ''}"
        ctk.CTkLabel(left_col, text=title_text, font=ctk.CTkFont(weight="bold", size=12)).pack(anchor="w")

        # Descrição/Didática
        ctk.CTkLabel(left_col, text=item.get("descricao", ""), font=ctk.CTkFont(size=10), text_color="#BDC3C7", wraplength=450, justify="left").pack(anchor="w")
        ...
        # Badge
        badge_val = item.get("risco") if modo == "risco" else item.get("impacto")
        badge_color = "#E74C3C" if "Alto" in str(badge_val) or "Queda" in str(badge_val) else "#2ECC71" if "Nenhum" in str(badge_val) else "#F39C12"
        ctk.CTkLabel(right_col, text=f" {badge_val} ", fg_color=badge_color, corner_radius=3, text_color="black", font=ctk.CTkFont(size=10)).pack(anchor="e", pady=(0, 5))

        # Botão
        btn_text = "Remover" if is_active else "Aplicar"
        ctk.CTkButton(right_col, text=btn_text, width=80, height=24, font=ctk.CTkFont(size=11), command=apply_callback).pack(anchor="e")
    ...
    def open_analysis_modal(self, analysis_data, account, is_cs2=True, on_success=None):
        ...
        for item in analysis_data:
            formatted_item = {
                "nome": item['name'],
                "descricao": f"Atual: {item['current']} | Ideal: {item['ideal']}",
                "impacto": item['impact']
            }
            ...
            def toggle_cb(v=var): v.set("1" if v.get() == "0" else "0")

            self.render_optimization_card(scroll_frame, formatted_item, toggle_cb, is_active=item['is_ok'], modo="performance")
    ...
    def open_launch_options_modal(self, account, current_options):
        ...
        for cmd, data in tesauro.items():
            ...
            self.render_optimization_card(scroll_frame, item, lambda c=cmd: toggle_command(c), is_active=is_active, modo="risco")
